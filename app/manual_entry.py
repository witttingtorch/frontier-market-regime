...
if st.sidebar.button("🔄 Fetch Latest FX Rates"):
    with st.spinner("Fetching..."):
        ...
        if not fx_df.empty:
            ...
            latest_date = (
                fx_df["date"].iloc[-1] if "date" in fx_df.columns else datetime.now()
            )
            st.session_state["fx_rate_value"] = latest_rate
            st.session_state["fetched_rate_display"] = (
                f"Rate: {latest_rate:.4f} | Date: {latest_date}"
            )
            st.rerun()
        else:
...
with tab1:
    ...
    with col1:
        ...
        spread_color = (
            "🟢"
            if interbank_spread_bps < 50
            else "🟡" if interbank_spread_bps < 150 else "🔴"
        )
        st.markdown(
            f"**Interbank Spread:** {spread_color} `{interbank_spread_bps} bps`"
        )

        policy_direction = st.selectbox(
            "Policy Direction", ["Stable", "Cutting", "Hiking"], key="policy_dir_input"
        )
    ...
    with col2:
        ...
        cpi_yoy = (
            st.number_input(
                "CPI YoY Inflation (%)", value=4.4, step=0.1, key="cpi_yoy_input"
            )
            / 100
        )
        cpi_mom = (
            st.number_input(
                "CPI MoM Change (%)", value=0.3, step=0.1, key="cpi_mom_input"
            )
            / 100
        )
    ...
    with col3:
        ...
        flow_z = st.number_input(
            "Capital Flow Z-Score", value=-0.3, step=0.1, key="flow_z_input"
        )
        flow_usd = st.number_input(
            "Net Flow (USD Millions)", value=-50.0, step=10.0, key="flow_usd_input"
        )
            ...
            with st.spinner("Calculating..."):
                ...
                variables = {
                    ...
                    "inflation": engine.score_inflation_momentum(
                        100 * (1 + cpi_yoy), 100 * (1 + cpi_yoy - cpi_mom), 100
                    ),
                ...
                regime, source, score = engine.compute_regime(
                    variables, regime_override
                )
...
with tab2:
    ...
    if st.session_state["result"]:
        ...
        col3.metric(
            ...
            (
                "High"
                if abs(result["score"]) >= 4
                else "Medium" if abs(result["score"]) >= 2 else "Low"
            ),
        )
            ...
            var_data.append(
                {
                    ...
                    "Value": (
                        f"{var.value:.2%}"
                        if "vol" in name.lower() or "inflation" in name.lower()
                        else f"{var.value:.2f}"
                    ),
        ...
        def color_score(val):
            ...
            if val == -1:
                return "background-color: #4a1020; color: #e74c3c"
            return "background-color: #4a3800; color: #f39c12"

        st.dataframe(
            df_vars.style.map(color_score, subset=["Score"]), use_container_width=True
        )
        ...
        tbill_spread = round(
            (inputs.get("tbill_91", 0) - inputs.get("policy_rate", 0)) * 100, 1
        )
...
with tab3:
    ...
    if not df_log.empty:
        ...
        if "interbank_spread_bps" in df_log.columns:
            ...
            st.line_chart(
                df_log.sort_values("date").set_index("date")["interbank_spread_bps"]
            )
        ...
        st.info(
            "No classifications saved yet. Use Data Entry tab to log your first entry."
        )
    ...
    with st.expander("📍 Add Street-Level Observation"):
        ...
        if observation:
            ...
            if matched:
                st.markdown(
                    f"**Detected signals:** `{', '.join(matched)}` → sentiment score: `{sentiment:+d}`"
                )

        obs_confidence = st.selectbox(
            "Confidence", ["Low", "Medium", "High"], key="obs_confidence"
        )
        ...
        if st.button("Log Observation", type="secondary"):
            if observation:
                ...
                try:
                    ...
                    updated = pd.concat(
                        [existing, pd.DataFrame([obs_record])], ignore_index=True
                    )
                except FileNotFoundError:
                    updated = pd.DataFrame([obs_record])
                updated.to_csv(obs_file, index=False)
                st.success(
                    f"✅ Observation logged — sentiment: {sentiment:+d} — {len(updated)} total"
                )
            else:
